...
export default function ScoringSummary({ scores, passingBenchmark }) {
  ...
  return (
    <section className="mt-6 border-2 border-slate-300 rounded-xl overflow-hidden print:rounded-none print:border-slate-700 print:mt-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-7 divide-x divide-y sm:divide-y-0 divide-slate-200 text-xs">
        ...
        <div className="p-3 text-center bg-white">
          <span className="text-slate-500 font-semibold block text-[11px]">Not Relevant</span>
          <span className="text-base font-bold text-slate-500">{scores.notRelevant}</span>
        </div>
        <div className="p-3 text-center bg-white">
      ...
      <div className="p-2.5 bg-slate-50 border-t border-slate-200 text-[11px] text-slate-600 italic">
        * Note: Items marked Not Relevant are excluded from the percentage score. If the final score is below{' '}
        <strong>{passingBenchmark}%</strong>, immediate corrective action review with the Store Area / Business Manager is mandatory
        within 24 hours.
      </div>
    </section>
  )
